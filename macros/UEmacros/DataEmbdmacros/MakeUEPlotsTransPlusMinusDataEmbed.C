...
void MakeUEPlotsTransPlusMinusDataEmbed(int n=1, int p=0)
{
  ...
  embdfile[12] = new TFile("/gpfs01/star/i_bnl/gdwebb/Run9/UEAnalysis/Embed_inclu/st_physics_75_-1.uehists.root");
   ...
   switch(n)
     ...
     case 1: // Average Track pT vs Jet pT
       {
	char name3[100]=Form("htransP_aveTrackpTVsMaxJetpT%s",trig);
	char name4[100]=Form("htransM_aveTrackpTVsMaxJetpT%s",trig);
	
	char embdname3[100]=Form("transPlus_aveTrackpTVsMaxJetpT%s",trig);
	char embdname4[100]=Form("transMiuns_aveTrackpTVsMaxJetpT%s",trig);
 ...
 case 8: // Average Max Track pT vs Jet pT
   ...
   TH2F* histogram_Trans = histogram_TranMax->Clone();
   ...
   TH2F* comboEmbdHist_Trans = comboEmbdHist_TranMax->Clone();
   ...
   if(n ==1){y1->SetTitle(" <p_{T,track}> (GeV)"); y1->SetRangeUser(0.4,0.8);}
   if(n ==2){y1->SetTitle("Track Multiplicity"); y1->SetRangeUser(1.5,4);}
